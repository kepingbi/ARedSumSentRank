...
#sort file name according to a numeric order so that 0, 1, 2, 10, 11, 90, instead of 0,1,10,11,2,90.
...
{elems}
    ...
    def write_config_static(system_dir, system_filename_pattern,
        ...
        system_filenames = sorted_aphanumeric(os.listdir(system_dir))
    ...
    def evaluate(self, system_id=1, rouge_args=None, show_all=True):
        """
        ...
        options = self.__get_options(rouge_args, show_all=show_all)
    ...
    #borrowed from rouge_papier
    def output_to_dataframe(self, output, max_ngram=2, lcs=True, return_conf=False):
        dataframes = []
        confs = []
        for r in range(1, max_ngram + 1):
            o, conf = self.convert_output(output, r)
            dataframes.append(o)
            confs.append(conf)
        if lcs:
            o, conf = self.convert_output(output, "L")
            dataframes.append(o)
            confs.append(conf)
    ...
    def convert_output(self, output, rouge=1):
        data = []
    ...
    def overall_output_to_dict(self, output):
        """
...
""".format(
    ...
    def __get_options(self, rouge_args=None, show_all=True):
        """
